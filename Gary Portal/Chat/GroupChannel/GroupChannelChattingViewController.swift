...
class GroupChannelChattingViewController: UIViewController, SBDConnectionDelegate, SBDChannelDelegate, ChattingViewDelegate, MessageDelegate, ConnectionManagerDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        if self.groupChannel.memberCount > 2 {
            self.navItem.title = self.groupChannel.name
        } else {
            for member in self.groupChannel.members! as! [SBDUser] {
                if member.userId != zeroPage.userStats.userUID {
                    self.navItem.title = member.nickname
                }
            }
        }
        ...
        for chat in zeroPage.userStats.protectedChats {
            if self.groupChannel.channelUrl == chat {
                isProtected = true
            }
        }
        
        if isProtected && !zeroPage.userStats.admin  {
            self.chattingView.readOnlyLabel.isHidden = false
            self.chattingView.messageTextView.isHidden = true
            self.chattingView.fileAttachButton.isHidden = true
            self.chattingView.placeholderLabel.isHidden = true
            self.chattingView.sendButton.isHidden = true
            self.chattingView.fileAttachPlus.isHidden = true
        } else {
            self.chattingView.readOnlyLabel.isHidden = true
            self.chattingView.messageTextView.isHidden = false
            self.chattingView.fileAttachButton.isHidden = false
            self.chattingView.placeholderLabel.isHidden = false
            self.chattingView.sendButton.isHidden = false
            self.chattingView.fileAttachPlus.isHidden = false
        }
        
        self.chattingView.channel = self.groupChannel
...
//        task.resume()
    ...
    func sendNotification(header: String, sub: String, main: String, group: Bool) {
    ...
    @objc private func sendMessage() {
        if self.chattingView.messageTextView.text.count > 0 {
            ...
            if self.groupChannel.memberCount > 2 {
                print("group")
                self.sendNotification(header: self.groupChannel.name, sub: "From: "+zeroPage.userStats.userName, main: message!, group: true)
            }
            else {
                self.sendNotification(header: "From: "+zeroPage.userStats.userName, sub: "", main: message!, group: false)
            }
        }
    }
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        ...
        if self.groupChannel.memberCount > 2 {
            self.sendNotification(header: self.groupChannel.name, sub: "From: "+zeroPage.userStats.userName, main: "Sent an image / video", group: true)
        }
        else {
            self.sendNotification(header: "From: "+zeroPage.userStats.userName, sub: "", main: "Sent an image / video", group: false)
        }
    }
